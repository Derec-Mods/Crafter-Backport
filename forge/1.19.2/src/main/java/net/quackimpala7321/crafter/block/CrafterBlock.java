...
public class CrafterBlock extends BlockWithEntity {
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return world.isClient ? null : checkType(type, ModBlockEntities.CRAFTER.get(), CrafterBlockEntity::tickCrafting);
    }
    ...
    private void transferOrSpawnStack(World world, BlockPos pos, CrafterBlockEntity blockEntity, ItemStack stack, BlockState state) {
        ...
        if (!itemStack.isEmpty()) {
            Vec3d vec3d = Vec3d.ofCenter(pos).add((double)direction.getOffsetX() * 0.7, (double)direction.getOffsetY() * 0.7, (double)direction.getOffsetZ() * 0.7);
