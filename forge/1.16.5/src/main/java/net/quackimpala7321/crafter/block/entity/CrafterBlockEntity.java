...
public class CrafterBlockEntity extends LootableContainerBlockEntity implements Tickable {
    ...
    public CrafterBlockEntity() {
        super(ModBlockEntities.CRAFTER.get());
    ...
    private boolean betterSlotExists(int count, ItemStack stack, int slot) {
        for(int i = slot + 1; i < 9; ++i) {
            if (!this.isSlotDisabled(i)) {
                ...
                if (itemStack.isEmpty() || itemStack.getCount() < count && itemStack.getItem() == stack.getItem() && ItemStack.areTagsEqual(itemStack, stack)) {
                    return true;
                }
            }
        }
